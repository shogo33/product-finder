/**
 * add-naturehike-products.mjs — Naturehike小物をDBに追加
 * 実行: node scripts/add-naturehike-products.mjs
 */
import fs from 'fs';
...
const TARGET_COUNT = 20; // Naturehike小物は絞って高品質に
...
function passesQuality(item) {
  ...
  // Naturehike小物: 300〜8000円、評価率85%以上、販売数30件以上
  return price >= 300 && price <= 8000 && rate >= 85 && sales >= 30 && !!item.product_main_image_url;
}
...
const NG_PATTERNS = [
  ...
  /\badult\b/i, /\bsexy\b/i, /\bknife\b/i,
  /\btent\b/i, /\bsleeping bag\b/i, /\bjacket\b/i,
];
...
async function cleanNamesBatch(titles) {
  ...
  try {
    const msg = await claude.messages.create({
      ...
      max_tokens: 400,
      messages: [{ role: 'user', content: `以下${titles.length}件のAliExpress商品名を、スマホカード表示用（最大20文字）に整理してください。ブランド名（Naturehike等）はできる限り残す。結果はJSON配列のみ出力。\n\n${titles.map((t, i) => `${i + 1}. ${t}`).join('\n')}` }],
    });
...
// Naturehike小物キーワード
const KEYWORDS = [
  'Naturehike camping pillow ultralight inflatable',
  'Naturehike compression stuff sack bag',
  'Naturehike tent peg stake ultralight',
  'Naturehike water bottle outdoor',
  'Naturehike dry bag waterproof',
  'Naturehike microfiber towel camp',
  'Naturehike camp cup titanium',
  'Naturehike sleeping bag liner',
  'Naturehike poncho rain ultralight',
  'Naturehike phone mount holder bike',
];

async function main() {
  console.log('════════════════════════════════════════');
  console.log('  Naturehike 小物 収集スクリプト');
  ...
  console.log('\n追加されたNaturehike商品:');
  newProducts.forEach(p => console.log(`  ${p.price}円 | ${p.name} | ${p.id}`));
  console.log('════════════════════════════════════════\n');
}
