...
#### 2. 「AI構文」と「誇大表現」の完全禁止
以下は【一切使用禁止】：
❌ AI構文：「〜と言えるでしょう」「〜が魅力的です」「〜を検討してみてはいかがでしょうか」「〜に定評があります」「〜の一品です」
❌ 禁止ワード：「コスパ最強」「圧倒的」「神」「革命的」「非常に優秀」「快適」「高性能」「最強」「完璧」「誰にでもおすすめ」「〜で有名」
❌ 断定レビュー：「使いました」「実際に試しました」（実機未使用の場合）
⭕ 代わりに：「Redditでは〜という声が多い」「AliExpressレビューでは〜という報告がある」「〜な人には向いている」「〜用途なら十分」「人によって好みが分かれる」「少し〜だが、このクラスではかなり〜」
...
#### 7. 記事冒頭の必須構造（この順で書く）
記事本文の一番最初に以下の2ブロックを必ず配置する：

①「結論」ブロック（比較表より前）：
\`\`\`html
<div class="article-conclusion">
  <p><strong>結論：</strong>○○用途なら【商品名A】、小型重視なら【商品名B】がおすすめ。</p>
  <ul>
    <li>【用途A】向け → 【商品名A】</li>
    <li>【用途B】向け → 【商品名B】</li>
  </ul>
...
③ 比較表は「こんな人向け」の直後に配置（本文後半ではなく冒頭付近）。

#### 8. デメリット・人間らしいノイズの必須記載
- 各商品セクションに**必ずデメリット・注意点**を書く（重い・発熱・箱潰れ・初期不良報告など）
- 「全員におすすめ」は禁止。用途別に向き不向きを書く
- 「少し〇〇だが、〜クラスではかなり〜」形式で現実感を出す
- 「写真より少し大きい」「思ったより発熱が少ない」など「想像とのズレ」を1箇所以上書く

#### 9. 本文中の内部リンク（記事末尾だけに頼らない）
- plan の internalLinks を記事末尾の関連記事だけでなく、**本文中にも自然に差し込む**
- 例：「充電器の比較は<a href="/gadget/ugreen-gan-charger-osusume.html">UGREEN充電器おすすめ記事</a>でも詳しく解説しています」`;
