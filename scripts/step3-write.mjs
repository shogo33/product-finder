...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* PCフローティングバナー */
    .pc-float-banner { display: none; position: fixed; bottom: 24px; right: 24px; width: 288px; background: #fff; border-radius: 16px; box-shadow: 0 8px 32px rgba(0,0,0,0.18); padding: 20px 20px 16px; z-index: 300; border: 1px solid rgba(0,0,0,0.07); animation: pcBannerIn 0.45s cubic-bezier(0.25,0.1,0.25,1); }
    @keyframes pcBannerIn { from { opacity: 0; transform: translateY(20px); } to { opacity: 1; transform: translateY(0); } }
    @media (min-width: 641px) { .pc-float-banner { display: block; } }
    .pc-float-close { position: absolute; top: 10px; right: 14px; background: none; border: none; font-size: 1.1rem; color: #aaa; cursor: pointer; line-height: 1; padding: 2px 6px; }
    .pc-float-close:hover { color: #333; }
    .pc-float-inner { display: flex; align-items: center; gap: 12px; margin-bottom: 14px; }
    .pc-float-icon { font-size: 2rem; flex-shrink: 0; }
    .pc-float-title { font-weight: 700; font-size: 0.95rem; margin: 0 0 3px; color: #111; line-height: 1.3; }
    .pc-float-desc { font-size: 0.76rem; color: #666; margin: 0; line-height: 1.45; }
    .pc-float-btn { display: block; background: var(--red); color: #fff !important; text-align: center; padding: 10px; border-radius: 8px; font-weight: 700; font-size: 0.88rem; text-decoration: none; transition: background 0.2s; }
    .pc-float-btn:hover { background: #c42800; }
...
<div class="pc-float-banner" id="pc-float-banner">
  ...
  <div class="pc-float-inner">
    <div class="pc-float-icon">👆</div>
    <div>
      <p class="pc-float-title">スワイプで商品を探す</p>
      <p class="pc-float-desc">あなたにぴったりのAliExpress商品が見つかる</p>
    </div>
  </div>
  <a href="/" class="pc-float-btn">試してみる →</a>
</div>
